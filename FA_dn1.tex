...
\emph{Rešitev}
\newline
Označimo s $T_i, i=1,2,\dots,$ elemente množice $\mathcal{F}$ in s $S_j, j=1,2,\dots$, elemente množice $\mathcal{G}$. Definiramo množice $A_{ij} = \{x\in X: T_i x = S_jx\} = \ker(T_i - S_j)$. Množice $A_{ij}$ so zaprte, saj gre ravno za jedro omejenega linearnega operatorja $T_i-S_j$. Očitno jih je tudi števno. Ker za vsak $x\in X$ obstajata operatorja $T_i$ in $S_j$, da je $T_i x = S_j x$, velja še $\cup_{i,j=1}^{\infty} A_{ij} = X$. Po posledici Baireovega izreka obstajata indeksa $i_0, j_0$, da ima množica $A_{i_0 j_0}$ neprazno notranjost.
