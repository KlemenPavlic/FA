...
\begin{enumerate}
\item[(a)]
\item[(b)] Denimo, da obstajata dve funckiji $f$ in $g$, da je $Af = Ag$. To pomeni, da za vsak $x\in [0,1]$ velja $(x-x^2) \int_0^x f(t)dt = (x-x^2) \int_0^xg(t)dt$. Od tod sledi, da mora biti $\int_0^x f(t)dt = \int_0^x g(t)dt$. Ker pa sta to odvedljivi funkciji, morati biti tudi njuna odvoda enaka. To implicira, da je $g(x) = f(x)$ za vsak $x\in[0,1]$, torej $g=f$. Sledi, da je $A$ injektiven operator.
