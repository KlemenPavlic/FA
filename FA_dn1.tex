...
\item[(b)] Trdim, da je $L^p(X) / L^p(A)$ izometrično izomorfen $L^p(A^C)$, kjer je $L^p(A^C)=\{f\in L^p(X): f=0\textrm{ skoraj povsod na } A\}$. Podobno kot zgoraj vidimo, da je tudi $L^p(A^C)$ zaprt podprostor $L^p(X)$ in zato Banachov.
...
$$
||f-g||^p = \int_X |f-g|^p d\mu = \int_A|f-g|^p d\mu + \int_{A^C} |f-g|^p d\mu.
$$
Če sedaj za $g$ izberemo $g=f\chi_A\in L^p(A)$, potem je zgornje naprej enako
$$
\int_A|f-f|^p d\mu + \int_{A^C} |f-0|^pd\mu = \int_{A^C}|f|^pd\mu = ||f\chi_{A^C}||^p.
$$
Sledi, da je $\varphi$ res izometrija.
\end{enumerate}
